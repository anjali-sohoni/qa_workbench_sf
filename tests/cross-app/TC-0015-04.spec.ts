...
test('TC-0015-04: Verify Application Number format compliance at maximum boundary', async () => {
  const testData = {
    "GPA__c": 3.72,
  ...
  expect(capturedAppNumber).toMatch(/^A-\d+$/);
  console.log('[test] Application Number format validation passed:', capturedAppNumber);
  ...
  console.log('[test] Application Number is visible and immutable on Sales detail page');
  console.log('[test] Application Number uniqueness verified via SOQL query');
  console.log('[test] All acceptance criteria validated successfully');
});