...
test('TC-0008-03: Verify Application Number format matches A-XXXXXX pattern exactly - Minimum GPA boundary', async () => {
  const testData = {
    Applicant_Name__c: 'Jordan Smith',
    Academic_Dept__c: 'Science',
    Application_Type__c: 'Under Graduate',
    FAFSA__c: true,
    GPA__c: 2.0,
    TestScore__c: 1520,
    School_Name__c: 'Acme Corporation',
    DOB__c: '03/22/1985',
    Email__c: 'synthetic.user.001@example.com'
  };
  ...
  const appNumMatch = pageText.match(/A-\d{6}/);
  const capturedAppNumber = appNumMatch?.[0] ?? '';
  console.log('[test] Captured Application Number:', capturedAppNumber);
  if (!capturedAppNumber) {
    throw new Error('Application Number not found in community page');
  }

  expect(capturedAppNumber).toMatch(/^A-\d{6}$/);
  ...
  await expect(salesPage.getByText(testData.TestScore__c.toLocaleString('en-US'), { exact: false }).first()).toBeVisible();
  await expect(salesPage.getByText(testData.GPA__c.toLocaleString('en-US'), { exact: false }).first()).toBeVisible();
  await expect(salesPage.getByText(testData.School_Name__c, { exact: false }).first()).toBeVisible();

  const appNumberLabel = salesPage.getByText('Application Number', { exact: false }).first();
  await expect(appNumberLabel).toBeVisible();

  await salesPage.goto(`${lightningOrigin}/lightning/r/${sfObjectApiName}/${sfRecordId}/edit`);
  await salesPage.waitForLoadState('domcontentloaded');
  await salesPage.waitForTimeout(2000);

  const nameFieldOnEdit = salesPage.locator('input[name="Name"]').first();
  await expect(nameFieldOnEdit).toBeDisabled();

  await salesPage.locator('button[name="CancelEdit"]').click();
  await salesPage.waitForLoadState('domcontentloaded');
});

test('TC-0008-03: Verify Application Number format matches A-XXXXXX pattern exactly - Maximum GPA boundary', async () => {
  const testData = {
    Applicant_Name__c: 'Taylor Chen',
    Academic_Dept__c: 'Business',
    Application_Type__c: 'Graduate',
    FAFSA__c: false,
    GPA__c: 4.0,
    TestScore__c: 950,
    School_Name__c: 'Global Industries',
    DOB__c: '11/08/1992',
    Email__c: 'synthetic.user.002@example.com'
  };
