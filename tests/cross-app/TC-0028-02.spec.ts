...
test('TC-0028-02: Submit form with FAFSA checkbox unchecked', async () => {
  const testData = {
    "FAFSA__c": false,
    "GPA__c": 3.42,
    "School_Name__c": "Horizon Academy Group",
    "TestScore__c": 1180,
    "Applicant_Name__c": "Meredith Grey",
    "Application_Type__c": "Under Graduate",
    "DOB__c": "7/14/1998",
    "Academic_Dept__c": "Business",
    "Email__c": "jordan.chen.592@example.com"
  };
  ...
  await commPage.locator(`[role="option"][data-value="${testData.Academic_Dept__c}"]`).click();

  const fafsaCheckbox = commPage.locator('input[name="FAFSA__c"]');
  await fafsaCheckbox.waitFor({ state: 'visible', timeout: 15000 });

  const isCheckedBefore = await fafsaCheckbox.isChecked();
  console.log('[test] FAFSA checkbox initial state:', isCheckedBefore);

  if (testData.FAFSA__c === false) {
    if (isCheckedBefore) {
  ...
  const isCheckedAfter = await fafsaCheckbox.isChecked();
  console.log('[test] FAFSA checkbox after interaction:', isCheckedAfter);
  expect(isCheckedAfter).toBe(testData.FAFSA__c);
  ...
  const capturedEmail = testData.Email__c;
  ...
  await expect(salesPage.getByText(capturedEmail, { exact: false }).first()).toBeVisible({ timeout: 30000 });
  ...
  console.log('[test] TC-0028-02 completed — Application created with FAFSA unchecked and verified in Sales');
});