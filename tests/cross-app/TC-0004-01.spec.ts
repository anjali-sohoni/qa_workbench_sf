...
test('TC-0004-01: Accept valid integer test score and submit form', async () => {
  const testData = {
    "FAFSA__c": true,
    "GPA__c": 3.85,
    ...
    "Email__c": "james.martinez@example.com"
  };
  ...
  await testScoreField.fill('abc');
  await commPage.waitForTimeout(1000);
  
  const errorMessage = commPage.getByText('Please enter a valid numeric score.', { exact: false });
  await expect(errorMessage.first()).toBeVisible({ timeout: 10000 });

  await testScoreField.clear();
