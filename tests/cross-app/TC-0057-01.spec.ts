...
test('TC-0057-01: Accept valid positive integer in Test Score field', async () => {
  const testData = {
    Email__c: `test.tc0057.${Date.now()}@example.com`,
    DOB__c: '01/15/1998',
    GPA__c: 3.75,
    TestScore__c: 1520,
    School_Name__c: 'Test High School',
  ...
  const testScoreField = commPage.locator('input[name="TestScore__c"]');
  await testScoreField.waitFor({ state: 'visible', timeout: 15000 });

  await testScoreField.fill('abc123');
  await commPage.waitForTimeout(1000);
  const validationMessage = commPage.getByText('Please enter a valid numeric score.', { exact: false });
  await expect(validationMessage).toBeVisible({ timeout: 5000 });

  await testScoreField.fill('');
  await testScoreField.fill(testData.TestScore__c.toString());
  await commPage.waitForTimeout(1000);
